    ...
    <div className="overflow-hidden">
      ...
      <section className="relative h-screen w-full flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-6">
          ...
          >
            ...
            />
            <h1 className="text-6xl md:text-9xl font-display text-white drop-shadow-lg tracking-tighter">
              {t.hero.title}
            </h1>
            <p className="text-xl md:text-2xl text-white font-light mt-4 drop-shadow-md">
              {t.hero.subtitle}
            </p>
          </motion.div>
        </div>
      ...
      <section className="py-20 px-6 bg-white/50">
        <div className="max-w-4xl mx-auto text-center border-y border-black/5 py-16">
          <h2 className="text-4xl font-display mb-8">{t.roots.title}</h2>
          <p className="text-xl font-light leading-loose opacity-80">
            {t.roots.description}
          </p>
        </div>
      </section>
      ...
      <section className="py-20 px-6 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-4">
          <div>
            <h2 className="text-4xl font-display">{t.status.title}</h2>
            <p className="text-lg opacity-60 mt-2">{t.status.description}</p>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {t.status.images.map((img: any, i: number) => (
            ...
            >
              <img
                src={`https://images.unsplash.com/photo-${i === 0 ? '1581091226825-a6a2a5aee158' : i === 1 ? '1460661419201-fd4cecdf8a8b' : '1504917595217-d4dc5f64968d'}?auto=format&fit=crop&q=80&w=800`}
                alt={img.alt}
                className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-700"
              />
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-6">
                <span className="text-white font-display text-xl">{img.title}</span>
              </div>
            </motion.div>
          ))}
        </div>
      ...
      <section className="py-20 px-6 max-w-7xl mx-auto mb-20">
        <div className="vibrant-card text-center bg-gradient-to-r from-gray-50 to-white">
          <h2 className="text-3xl font-display mb-4">{t.partners.title}</h2>
          <p className="opacity-60 mb-12">{t.partners.description}</p>
          <div className="flex flex-wrap justify-center gap-8 md:gap-16 opacity-40 grayscale filter">
            {t.partners.list.map((partner: string, i: number) => (
              <span key={i} className="text-lg font-display uppercase tracking-widest hover:opacity-100 transition-opacity cursor-default">
                {partner}
              </span>
            ))}
          </div>
