    ...
    <section className="p-6 md:p-12 max-w-7xl mx-auto py-20" data-sb-object-id="content/translations.json">
      <div className="mb-20">
        ...
        <h1 
          ...
          data-sb-field-path={`${language}.journal.title`}
        >
          {t.journal.title}
        </h1>
        <p 
          className="text-sm uppercase tracking-[0.5em] opacity-40"
          data-sb-field-path={`${language}.journal.wip`}
        >
          {t.journal.wip}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-16 lg:gap-24">
        {t.journal.posts.map((post: any, i: number) => {
          return (
            <div key={i} data-sb-field-path={`${language}.journal.posts.${i}`}>
              <JournalPost
                index={i}
                date={post.date}
                title={post.title}
